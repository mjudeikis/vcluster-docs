...
synced, they are read-only. No changes to the resource in the virtual cluster syncs back to the host cluster as the 
...
Several labels are applied to the host cluster by vCluster that never get synced to the virtual cluster resource. These labels are: 
